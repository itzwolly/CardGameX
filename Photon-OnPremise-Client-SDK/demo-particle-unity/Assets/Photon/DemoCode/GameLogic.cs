    ...
    public class GameLogic : LoadBalancingClient
    {
        ...
        private void SendPositionUpdate()
        {
            if (this.UseInterestGroups)
            {
                ...
                this.loadBalancingPeer.OpRaiseEvent(DemoConstants.EvPosition, this.LocalPlayer.WriteEvMove(), this.SendReliable, new RaiseEventOptions() { InterestGroup = playerGroup });
            }
            else
            {
                // this overload of OpRaiseEvent sends to everyone in the room - even those who didn't subscribe to any group
                this.loadBalancingPeer.OpRaiseEvent(DemoConstants.EvPosition, this.LocalPlayer.WriteEvMove(), this.SendReliable, null);
            }
        }
        ...
        public void CreateParticleDemoRoom(DemoConstants.MapType maptype, int gridSize)
        {
            ...
            this.OpJoinOrCreateRoom("Demo", new RoomOptions() { CustomRoomProperties = roomPropsForCreation, CustomRoomPropertiesForLobby = RoomPropsInLobby }, null);
        }
    }
