...
namespace Photon.Hive.Plugin
{
    ...
    public class PluginBase : IGamePlugin
    {
        ...
        protected void RaiseEvent(byte eventCode, object eventData, IList<int> targetActorsNumbers,
            int senderActorNumber = 0,
            byte cachingOption = CacheOperations.DoNotCache,
            SendParameters sendParams = default(SendParameters)) 
        {
            PluginHost.BroadcastEvent(targetActorsNumbers, senderActorNumber, eventCode, new Dictionary<byte, object>() { { 245, eventData }, { 254, senderActorNumber } }, cachingOption, sendParams);
        }

        protected void RaiseEvent(byte eventCode, object eventData,
            ...
            SendParameters sendParams = default(SendParameters)) 
        {
            PluginHost.BroadcastEvent(receiverGroup, senderActorNumber, interestGroup, eventCode, new Dictionary<byte, object>() { { 245, eventData }, { 254, senderActorNumber } }, cachingOption, sendParams);
        }
